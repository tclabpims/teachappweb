...
<div class="nr">
    ...
    <div class="pagemain">
        ...
        <input id="strSubMenuId" type="hidden" value="5"/>
        <%--查询 start--%>
        <div>
            <br/>
            <form class="layui-form" action="${pageContext.request.contextPath}/rateStat/excellentStat.do" METHOD="post">
                &nbsp;&nbsp;&nbsp;
        ...
        <h3 align="center">
            <c:choose><c:when test="${semester == null}">${year}</c:when><c:otherwise>${semester}</c:otherwise></c:choose>年度（或学期）各系督导评价优秀占比表
        </h3>
        <%--<hr/>--%>
        <table class="sui-table table-bordered" style="margin-top:20px;" id="schoolStat">
            ...
            <thead>
                <tr>
                    ...
                    <th class="center left-con" width="70%" style="vertical-align: middle" colspan="${fn:length(list)}">90分以上 (优秀,人数)</th>
                ...
                <tr>
                    <c:forEach var="item" items="${list}">
                        ...
                        <script>
                            var o = {};
                            o.name = "${item.xiName}" + "\n" + "${(item.excellentPerCent * 100)}" + '%';
                            o.value = parseInt("${item.excellentNumber}");
                            data[i++] = o;
                        </script>
                    </c:forEach>
                </tr>
            ...
            <tr>
                ...
                <c:forEach var="item" items="${list}">
                    <c:set var="totalPerCent" value="${totalPerCent + item.excellentPerCent}"></c:set>
                    <td class="center left-con"><fmt:formatNumber type="number" value="${item.excellentPerCent * 100}" pattern="#0.00" maxFractionDigits="2"></fmt:formatNumber>%</td>
                </c:forEach>
...
<%--分页 end--%>
<script type="text/javascript" src="${pageContext.request.contextPath}/statics/js/rateStat/excellentStat.js"></script>
</body>
</html>
